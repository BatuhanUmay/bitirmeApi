...
def calculate_arabam():
    ...
    tyle Red Skypack': 69, '1.33 Style Skypack': 70, '1.4': 71, '1.4 Alize': 72, '1.4 Authentique': 73,
                  ...
                  '1.4 CRDi Team': 78, '1.4 CVVT Cool': 79, '1.4 Classic': 80, '1.4 Comfort': 81, '1.4 Comfortline': 82, '1.4 D-4D Advance': 83,
    ...
    dict_KasaTipi = {'Coupe': 0, 'Hatchback/3': 1,
                     'Hatchback/5': 2, 'MPV': 3, 'Sedan': 4, 'Station wagon': 5}
...
def calculate_emlak():
    ...
    df[['il', 'ilce', 'mahalle']] = df['kisa_isim'].str.split(
        ' - ', expand=True)
    ...
    df["aidat"] = df["aidat"].apply(
        lambda x: str(x).strip("TL").replace(" ", ""))
    df["brut_metrakere"] = df["brut_metrakere"].apply(
        lambda x: str(x).replace(" M2", "").replace(".", ""))
    df["net_metrakere"] = df["net_metrakere"].apply(
        lambda x: str(x).replace(" M2", "").replace(".", ""))
    df["banyo_sayisi"] = df["banyo_sayisi"].apply(
        lambda x: str(x).strip("+").replace("Yok", "0"))
    df["oda_sayisi"] = df["oda_sayisi"].apply(
        lambda x: str(x).strip(".").replace("Oda", "")[-3:])
    df["yasi"] = df["yasi"].apply(lambda x: str(
        x).split("-")[0].strip(" Ve Üzeri (Yeni)"))
    ...
    df['total_rooms'] = df['oda_sayisi'].apply(
        toplam_ayir_ve_hesapla) + df["banyo_sayisi"]
    df["brut_div_net"] = df["brut_metrakere"] / df["net_metrakere"]
    df['rooms_per_bathroom'] = df['banyo_sayisi'] / df['total_rooms']
    df['air_conditioning'] = np.where(
        df['isitma_tipi'].isin(['Isıtma Yok']), 'yok', 'var')
    ...
    binary_cols = [col for col in df.columns if df[col].dtypes ==
                   "O" and len(df[col].unique()) == 2]
    df_copy = df.copy()
